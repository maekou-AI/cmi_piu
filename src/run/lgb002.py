...
def train_kfold(features, targets, categorical_features, n_splits, model_params, weights=None):
    ...
    for trn_idx, val_idx in skf.split(features, targets):
        ...
        losses.append(
            eval_result["val"]["rmse"][booster.best_iteration - 1]
        )
        oof_preds[val_idx] = booster.predict(features.iloc[val_idx])
    print(f"Avg. RMSE: {np.mean(losses):.4f}")
    return boosters, eval_result, losses, oof_preds
...
def run_kfold(
    ...
    pd.DataFrame(data={"id": trn_id, "sii": oof_preds}).to_csv(save_dir / "oof_prediction_raw.csv", index=False)
